...
class AudioAlarmManager:
    ...
    def _worker(self) -> None:
        ...
        dpath = self._base / self._drowsy_name
        ppath = self._base / self._dist_name
        drowsy_sound = None
        dist_sound = None
        ...
        if dpath.is_file():
            ...
            logger.warning("Không tìm thấy file âm thanh: %s — bỏ qua BUỒN NGỦ WAV.", dpath)
        ...
        if ppath.is_file():
            ...
            logger.warning("Không tìm thấy file âm thanh: %s — bỏ qua MẤT TẬP TRUNG WAV.", ppath)
            ...
            if desired != playing:
                ...
                if desired == "DROWSY":
                    if drowsy_sound is not None:
                        ...
                        logger.warning("BUỒN NGỦ nhưng không có WAV hợp lệ — không phát âm thanh.")
                    ...
                    if dist_sound is not None:
                        ...
                        logger.warning("MẤT TẬP TRUNG nhưng không có WAV hợp lệ — không phát âm thanh.")
