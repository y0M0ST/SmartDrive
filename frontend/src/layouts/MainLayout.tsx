...
export default function MainLayout() {
  ...
  return (
    ...
    <div className="flex min-h-screen bg-slate-50 dark:bg-slate-950 antialiased transition-colors duration-300">
      ...
      <aside className="w-72 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col p-6 fixed h-full z-40 transition-colors">
          ...
          {MENU_ITEMS.map((item) => {
            ...
            return (
              <Link 
                key={item.path} 
                to={item.path} 
                className={`flex items-center gap-3 px-4 py-3 rounded-xl font-bold text-[13px] transition-all ${
                  isActive 
        ...
        <div className="mt-auto p-4 bg-slate-50 dark:bg-slate-800/40 rounded-2xl border border-slate-100 dark:border-slate-800">
          <p className="text-[11px] font-extrabold text-slate-900 dark:text-white uppercase tracking-wider">Safe Drive System</p>
          <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5 font-medium">Version: 1.0.0.11</p>
        </div>
      </aside>
      ...
      <main className="flex-1 ml-72 flex flex-col min-h-screen">
        ...
        <header className="h-20 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-8 sticky top-0 z-50 transition-colors">
          <div className="relative w-80 group">
            ...
            <input 
              className="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl h-10 pl-10 text-sm outline-none transition-all dark:text-slate-200 dark:placeholder-slate-500" 
          ...
          <div className="flex items-center gap-5">
            <div className="flex items-center gap-4 text-slate-400 border-r pr-5 border-slate-200 dark:border-slate-800">
              <span className="text-sm font-bold text-slate-700 dark:text-slate-300 mr-1">Theme</span>
              ...
              >
                <div className={`absolute top-1 w-3 h-3 rounded-full transition-all ${isDark ? 'right-1 bg-blue-500' : 'left-1 bg-white'}`}></div>
              </button>

              <Icons.Mail size={20} className="hover:text-blue-600 dark:hover:text-blue-400 cursor-pointer transition-colors" />
              <div className="relative">
                <Icons.Bell size={20} className="hover:text-blue-600 dark:hover:text-blue-400 cursor-pointer transition-colors" />
            ...
            <DropdownMenu>
              <DropdownMenuTrigger className="outline-none">
                <div className="flex items-center gap-3 hover:bg-slate-50 dark:hover:bg-slate-800 p-1.5 rounded-2xl transition-all group">
                  <div className="w-10 h-10 bg-slate-100 dark:bg-slate-800 rounded-full border-2 border-white dark:border-slate-700 overflow-hidden flex items-center justify-center">
                    <Icons.UserCircle className="text-slate-400 w-full h-full" strokeWidth={1} />
                  </div>
                  <div className="text-left">
                    <p className="text-sm font-bold text-slate-900 dark:text-white leading-none">Admin</p>
                  </div>
                  <Icons.ChevronDown size={16} className="text-slate-400 group-data-[state=open]:rotate-180 transition-transform" />
                </div>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-56 mt-2 rounded-2xl shadow-2xl border-slate-100 dark:border-slate-800 p-2 bg-white dark:bg-slate-900">
        ...
        <div className="p-8 flex-1 bg-slate-50 dark:bg-slate-950 transition-colors">
          <Outlet />
        </div>
      </main>
    </div>
