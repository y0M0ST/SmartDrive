...
import { createAgencyService, deleteAgencyService, getAgencyByIdService, listAgenciesService, updateAgencyService } from './agency.service';

export const listAgencies = async (req: AuthRequest, res: Response) => {
  try {
    const rows = await listAgenciesService(req.user!);
    return res.status(200).json({ success: true, message: 'Lấy danh sách đại lý thành công', data: rows } as ApiResponse);
  } catch (err) {
...
export const getAgencyById = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const result = await getAgencyByIdService(agencyId, req.user!);
    switch (result.code) {
      case 'FORBIDDEN':
...
export const createAgency = async (req: AuthRequest, res: Response) => {
  try {
    ...
    switch (result.code) {
      ...
      case 'USER_NOT_FOUND':
        return res.status(404).json({ success: false, message: 'Không tìm thấy tài khoản quản lý', error: 'USER_NOT_FOUND' } as ApiResponse);
      default:
  ...
  try {
    ...
    switch (result.code) {
      ...
      case 'AGENCY_HAS_LINKED_DATA':
        return res.status(409).json({ success: false, message: 'Không thể xóa đại lý vì còn tài xế hoặc xe liên kết', error: 'AGENCY_HAS_LINKED_DATA', data: result.data } as ApiResponse);
      default:
