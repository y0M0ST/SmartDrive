  ...
  try {
    ...
    switch (result.code) {
      ...
      case 'DUPLICATE_EMAIL':
        return res.status(409).json({ success: false, message: 'Email tài khoản đã tồn tại trong hệ thống', error: 'DUPLICATE_EMAIL' } as ApiResponse);
      default:
...
export const resetDriverAccountPassword = async (req: AuthRequest, res: Response) => {
  try {
    const accountId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
    const result = await resetDriverAccountPasswordService(accountId, req.admin!);
...
export const getMyDriverAccount = async (req: AuthRequest, res: Response) => {
  try {
    if (req.admin?.account_type !== 'driver') {
      return res.status(403).json({ success: false, message: 'API nay chi danh cho tai xe', error: 'FORBIDDEN' } as ApiResponse);
    }

    const result = await getMyDriverAccountService(req.admin.id);
    if (result.code === 'NOT_FOUND') {
      return res.status(404).json({ success: false, message: 'Khong tim thay tai khoan tai xe', error: 'NOT_FOUND' } as ApiResponse);
    }

    return res.status(200).json({ success: true, message: 'Lay thong tin tai khoan thanh cong', data: result.data } as ApiResponse);
  } catch (err) {
    console.error('[DriverAccounts] GetMe error:', err);
    return res.status(500).json({ success: false, message: 'Loi he thong, vui long thu lai sau', error: 'INTERNAL_ERROR' } as ApiResponse);
  }
};

export const updateMyDriverAccountEmail = async (req: AuthRequest, res: Response) => {
  try {
    if (req.admin?.account_type !== 'driver') {
      return res.status(403).json({ success: false, message: 'API nay chi danh cho tai xe', error: 'FORBIDDEN' } as ApiResponse);
    }

    const result = await updateMyDriverAccountEmailService(req.admin.id, req.body.email);
    ...
    switch (result.code) {
      ...
      default:
        return res.status(200).json({ success: true, message: 'Cap nhat email thanh cong', data: result.data } as ApiResponse);
    }
  } catch (err) {
    console.error('[DriverAccounts] UpdateMyEmail error:', err);
    return res.status(500).json({ success: false, message: 'Loi he thong, vui long thu lai sau', error: 'INTERNAL_ERROR' } as ApiResponse);
  }
};