...
const validateVehicleFields = (
  ...
  if (Object.prototype.hasOwnProperty.call(payload, 'camera_code') && payload.camera_code !== null) {
    if (typeof payload.camera_code !== 'string') {
      res.status(400).json({
        success: false,
        message: 'Mã Camera AI không hợp lệ',
        error: 'INVALID_CAMERA_CODE',
      } as ApiResponse);
      return false;
    }

    if (payload.camera_code.trim().length > 255) {
      res.status(400).json({
        success: false,
        message: 'Mã Camera AI không được vượt quá 255 ký tự',
        error: 'INVALID_CAMERA_CODE',
      } as ApiResponse);
      return false;
    }
